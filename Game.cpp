...
Game::Game()
{
  ...
  winText = new HudText(-1, -1, "win");
  loseText = new HudText(-1, -1, "lose");
  timerText = new HudText(40, 650, "0");
  lifeText = new HudText(550, 650, liv.str());
...
void Game::createEnemyRandom()
{
  ...
  for (int i=0; i<20; i++)
  {
    ...
    while (collisionWithWall(x, y, 1.0) /*&& collisionWithEnemy(x, y, 1.0)*/)
	calcolaCoordinate(x, y, 10, 100);
...
bool Game::collisionPlayerWithEnemy(float eyeX, float xPlayer, float eyeY, float yPlayer)
{
  ...
  for (int i = 0; i < enemy.size(); i++)
  {
	...
	(eyeY - yPlayer >= enemy[i]->getY() && eyeY - yPlayer <= enemy[i]->getY() + enemy[i]->getSize().width)))
    {      
      while (collisionWithWall(x, y, enemy[i]->getSize().width) && collisionWithEnemy(x, y, enemy[i]->getSize().width))
	calcolaCoordinate(x, y, 10, 100);
